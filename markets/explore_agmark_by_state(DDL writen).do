...
gen normalized_output = (output / output_normalizer) /*this is output(as an item's daily quantity * item's av price in 2018) / total output for that week in 2018, same units cancel out*/
...
/* export graph */
graph save regional_perish_combined
graph export regional_perish_combined.png
...
/* graph perishable good output by year, looping through regions */
forvalues i = 1/`r(max)' {
  twoway (line log_normalized_output week if year == 2019 & group == `i') ///
      (line log_normalized_output week if year == 2020 & group == `i'), ///
      title(`i') ///
...
graph save regional_nonperish_combined
graph export regional_nonperish_combined.png
